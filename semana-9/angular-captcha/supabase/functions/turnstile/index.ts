...
//
// Mismo esquema que la función `captcha` (reCAPTCHA v2), pero contra Cloudflare:
// el widget del navegador resuelve el reto y entrega un token al frontend, y
// esta función es el único paso de servidor que confirma si ese token es válido.
//
//   Angular  --token-->  esta función  --token + secret-->  Cloudflare
//                                      <--- { success } ---
//
...
async function siteverify(
  ...
  // Permite reintentar la verificación del mismo token si la red falla:
  // sin idempotency_key el segundo intento daría 'timeout-or-duplicate'.
...
async function verify(req: Request): Promise<Response> {
  ...
  if (!secret) {
    console.error("Falta la variable de entorno TURNSTILE_SECRET");
  ...
  // Cloudflare invalida el token tras el primer canje: si alguien reenvía el
  ...
  if (
    hostnames.size > 0 && (!result.hostname || !hostnames.has(result.hostname))
  ) {
    return json({ valid: false, reason: "hostname_mismatch" }, 403);
  }

  // El widget puede etiquetar cada reto con un `action`. Si el cliente dice qué
  // acción esperaba, se comprueba que sea la que Cloudflare firmó: así un token
  // obtenido en el login no sirve para el formulario de pago.
  if (typeof action === "string" && action && result.action !== action) {
    return json({ valid: false, reason: "action_mismatch" }, 403);
  }

  return json({ valid: true, action: result.action, challengeTs: result.challenge_ts });
}
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/turnstile' \
